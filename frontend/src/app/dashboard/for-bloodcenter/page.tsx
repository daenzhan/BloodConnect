...
export default function BloodReservePage() {
    ...
    const totalUnits = allReserves.length;
    const availableUnits = allReserves.filter(r => r.isReady && r.daysUntilExpiration > 7).length;
    const quarantineUnits = allReserves.filter(r => r.inQuarantine).length;
    const expiringUnits = allReserves.filter(r => r.daysUntilExpiration <= 7 && r.daysUntilExpiration > 0).length;
    const expiredUnits = allReserves.filter(r => r.daysUntilExpiration <= 0).length;
            ...
            <main className="ml-20 lg:ml-64 p-6 lg:p-8 min-h-screen overflow-auto bg-background">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                    ...
                    <Card className="p-4 text-center bg-yellow-50 border-yellow-200">
                        <p className="text-2xl font-bold text-yellow-600">{quarantineUnits}</p>
                        <p className="text-xs text-yellow-600">Quarantine</p>
                    </Card>
                    <Card className="p-4 text-center bg-orange-50 border-orange-200">
                        <p className="text-2xl font-bold text-orange-600">{expiringUnits}</p>
                        <p className="text-xs text-orange-600">Expiring Soon</p>
                    </Card>
                    <Card className="p-4 text-center bg-red-50 border-red-200">
                        <p className="text-2xl font-bold text-red-600">{expiredUnits}</p>
